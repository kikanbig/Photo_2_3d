#!/usr/bin/env python3
"""
RunPod API сервер для EmbodiedGen Image-to-3D
Использует готовый образ wangxinjie/embodiedgen:v0.1.x
"""
...
log_file = log_dir / f"embodiedgen_api_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Логируем запуск
logger.info("=" * 80)
logger.info("🚀 EmbodiedGen RunPod API Server Starting")
...
def handler(event: Dict[str, Any]) -> Dict[str, Any]:
    """
    Основной обработчик запросов RunPod для EmbodiedGen
    """
    ...
    try:
        ...
        logger.info(f"🔧 img3d-cli executable: {subprocess.which('img3d-cli')}")
        
        logger.info("⏳ Running EmbodiedGen subprocess...")
        ...
        if result.returncode != 0:
            ...
            return {
                "error": f"Ошибка EmbodiedGen: {result.stderr}",
        ...
        response = {
            "success": True,
            "message": "3D модель успешно сгенерирована с помощью EmbodiedGen",
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        try:
            ...
            print("Запуск EmbodiedGen RunPod сервера...")
